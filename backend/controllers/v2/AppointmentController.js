...
const bookAppointment = async (req, res) => {
  try {
    ...
    if (!authority_id || !time_slot || !date) {
      return res
        .status(400)
        .json({ error: "Authority ID, time slot, and date are required" });
    }
    ...
    if (!timeSlot) {
      return res
        .status(404)
        .json({ error: "Time slot not found for the specified date" });
    }
    ...
    try {
      await notificationService.sendConfirmationEmail(newAppointment);
    } catch (emailError) {
...
const cancelAppointment = async (req, res) => {
  try {
    ...
    if (!reason) {
      return res
        .status(400)
        .json({ error: "Descriptive reason for cancellation is required" });
    }
    ...
    try {
      const cancelledBy = "official" in req ? "official" : "user";
...
const addAttendeeToAppointment = async (req, res) => {
  try {
    ...
    if (!appointment_id || !nic || !name) {
      return res
        .status(400)
        .json({ error: "Appointment ID, NIC, and name are required" });
    }
...
const getAttendeesOfAppointment = async (req, res) => {
  try {
    ...
    if (!attendees || attendees.length === 0) {
      return res
        .status(404)
        .json({ error: "No attendees found for this appointment" });
    }
...
const addAttachmentFile = async (req, res) => {
  try {
    ...
    if (!appointment_id || !file_url) {
      return res
        .status(400)
        .json({ error: "Appointment ID and file URL are required" });
    }
...
const getAttachmentByAppointmentId = async (req, res) => {
  try {
    ...
    if (!attachment) {
      return res
        .status(404)
        .json({ error: "Attachment not found for this appointment" });
    }
...
const removefileFromAttachment = async (req, res) => {
  try {
    ...
    if (!attachment.file_urls.includes(file_url)) {
      return res
        .status(404)
        .json({ error: "File URL not found in the attachment" });
    }
...
// New function: Send official comment/update to citizen
const sendOfficialUpdate = async (req, res) => {
  try {
    const { appointment_id, message, update_type } = req.body;
    ...
    // Update appointment with official comment if it's a comment
    if (update_type === "comment") {
      await prisma.Appointment.update({
        ...
        data: {
          official_comment: message,
        },
      });
    }

    // Send email notification based on update type
    try {
      switch (update_type) {
        case "comment":
          await notificationService.sendCommentEmail(
            parseInt(appointment_id),
            message,
            official.username || "Government Official"
          );
          break;
        case "request":
          await notificationService.sendDocumentRequestEmail(
            parseInt(appointment_id),
            message,
            official.username || "Government Official"
          );
    ...
    res.status(200).json({
      message: `${
        update_type.charAt(0).toUpperCase() + update_type.slice(1)
      } sent successfully`,
      appointment_id: appointment_id,
      update_type: update_type,
    });
  } catch (error) {
    console.error("Error sending official update:", error);
    return res.status(500).json({ error: "Internal Server Error" });
  }
};

// New function: Reschedule appointment with email notification
const rescheduleAppointment = async (req, res) => {
  try {
    const { appointment_id, new_date, new_time_slot, reason } = req.body;
    const { official } = req; // Extract official info from the request object
    ...
    const newTimeSlot = await prisma.Free_Times.findFirst({
      where: {
        authority_id: official.authority_id,
    ...
    if (!newTimeSlot) {
      return res
        .status(404)
        .json({ error: "New time slot not available for the specified date" });
    }
    ...
    const oldFreeTimesRecord = await prisma.Free_Times.findFirst({
      where: {
        authority_id: appointment.authority_id,
        date: appointment.date,
      },
    ...
    if (!oldFreeTimesRecord) {
      await prisma.Free_Times.create({
        data: {
          authority_id: appointment.authority_id,
          date: appointment.date,
          time_slots: [appointment.time_slot],
        },
      });
    } else {
      const updatedTimeSlots = [
        ...oldFreeTimesRecord.time_slots,
        appointment.time_slot,
      ];
      await prisma.Free_Times.update({
        where: {
    ...
    // Remove the new booked time slot
    if (newTimeSlot.time_slots.length === 1) {
      await prisma.Free_Times.delete({
        where: {
          authority_id_date: {
            authority_id: official.authority_id,
      ...
      await prisma.Free_Times.update({
        ...
        data: {
          time_slots: {
            set: newTimeSlot.time_slots.filter(
              (slot) => slot !== new_time_slot
            ),
          },
        },
      });
    ...
    try {
      await notificationService.sendRescheduleEmail(
        parseInt(appointment_id),
        new Date(new_date),
        new_time_slot,
        reason,
